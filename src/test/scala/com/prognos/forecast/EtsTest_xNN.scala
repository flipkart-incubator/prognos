...
class EtsTest_xNN extends FlatSpec with Matchers {
  it should "testing ANN, MNN" in {
    ...
    val (forecastsANN, _) = algo.calculateETS(series, alpha, beta, gamma, "ANN", period, horizon)
    val (forecastsMNN, _) = algo.calculateETS(series, alpha, beta, gamma, "MNN", period, horizon)
